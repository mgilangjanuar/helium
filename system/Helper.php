...
class Helper
{
    ...
    {
        $value = str_replace('@webroot', $_SERVER['DOCUMENT_ROOT'] . App::$url->path(), $value);
        $value = str_replace('@web', App::$url->baseUrl(), $value);
        return $value;
    }

    public function generateRandomString($length = 10, $withChars = true)
    {
        $string = "0123456789abcdefghijklmnopqrstsouvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
        if ($withChars) {
            $string .= "~!@#$%^&*()_-+=[]{}|;:<>,./?";
        }
